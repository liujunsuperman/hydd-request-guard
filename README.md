...
| 治理能力 | 状态 | 说明 |
|----------|------|------|
| 防重复请求（Duplicate） | ✅ 已内置 | ignore 静默忽略、block 阻断或 reuse 复用重复请求 |
...
// 快速双击提交，第二次请求默认会被 ignore：不发请求，也不触发 then/catch
```
...
const requestManager = setupRequestGuard({
  notify: (payload) => Toast.show(payload.message),
  rules: [
    { method: 'post', duplicate: { strategy: 'ignore', message: '正在处理' } },
    { url: /\/api\/config/, retry: { attempts: 3, delay: 1000 } }
  ]
});

// 后续可动态管理
requestManager.addRule({ url: /\/api\/payment/, duplicate: { strategy: 'ignore' } });
...
防止同一请求被重复发送。默认 `ignore` 会让重复请求静默挂起，避免触发调用方 `.then()` / `.catch()` 中的业务副作用；需要显式错误时用 `block`，查询合并时用 `reuse`。

| 策略 | 首次请求 | 重复请求 | 适合场景 |
|------|----------|----------|----------|
| **ignore**（忽略，默认） | 正常发送 | 返回永远 pending 的 Promise，不触发 `.then()` / `.catch()` / `.finally()` | 表单提交、下单、支付、已有错误/成功副作用的项目 |
| **block**（阻断） | 正常发送 | 直接拒绝，抛出 `RequestGuardBlockedError` | 需要业务显式感知重复提交 |
| **reuse**（复用） | 正常发送 | 等待首次请求结果，共享响应 | 列表查询、数据加载、配置查询 |

```javascript
// ignore：默认表单提交防双击，重复调用不会进入 then/catch
...
| inFlightTtl | `0` | 在途记录兜底存活时间，毫秒；默认只在原请求 settle 或 `clearState()` 时释放 |
| message | `'请勿重复提交'` | 默认提示文案 |
| keyGenerator | `null` | 自定义 key 生成函数 |

`ignore` 返回的 Promise 永远不会 settle。request-guard 内部不会保存这份 Promise，因此调用方释放引用后可由 GC 回收；但如果业务代码 `await` 了这个重复请求，该 async 执行流会一直挂起，这是 `ignore` 的语义代价。组件卸载、页面离开和业务取消应由调用方自己的生命周期处理。

从旧版本升级时，如果项目依赖默认 `block` 的 `.catch(RequestGuardBlockedError)` 来关闭 loading、重置按钮或展示提示，请显式配置 `defaults.duplicate.strategy: 'block'`，或在请求级写 `requestGuard: { duplicate: { strategy: 'block' } }`。如果使用自定义 transport，且存在请求 Promise 永不 settle 的风险，可以配置 `inFlightTtl` 作为兜底清理；开启后，超时的原请求仍未结束时，后续同 key 请求可能重新进入真实 transport。
...
setupRequestGuard(axios, {
  rules: [
    { url: '/api/order/submit', duplicate: { strategy: 'ignore' } },          // 精确匹配
...
| 触发 | capability | 说明 |
|------|-----------|------|
| duplicate 命中 | `duplicate` | ignore / block / reuse 命中重复请求均触发 |
| 熔断拦截 | `circuitBreaker` | 熔断打开，请求被拒绝 |
| 熔断状态变更 | `circuitBreaker` | 状态在 NORMAL/CIRCUIT_BREAKER/HALF_OPEN 间切换 |
| retry | — | 默认不触发 notify，仅走 logger |

duplicate 的 `payload.message` 保持为配置文案本身；重复命中次数通过 `payload.repeatCount` 传出。
...
推荐统一用 `RequestGuardErrorName` 和 `error.name` 判别：

```javascript
import { RequestGuardErrorName } from '@hydd/request-guard';

try {
  await request();
} catch (error) {
  if (error.name === RequestGuardErrorName.BLOCKED) {
    // duplicate block 策略命中
  }
}
```

`RequestGuardErrorName` 是冻结的公共枚举式对象，内部运行时不会复用这份对象作为能力私有状态。
...
::: warning 短路错误不经过响应拦截器
duplicate block、熔断拦截等"请求未发出"的短路错误，**不会进入 `axios.interceptors.response.use`**。用 `notify` 出口或请求级 `.catch` 处理。duplicate ignore 命中本身不会产生错误；只有容量硬上限等内部保护才会抛出 `RequestGuardCacheEvictedError`。
:::
...
setupRequestGuard(axios, {
  notify: (payload) => Toast.show(payload.message),
  defaults: { duplicate: { strategy: 'ignore', message: '订单处理中，请勿重复提交' } }
});
...
// 重复提交不会发出请求，也不会触发 then/catch/finally 中的业务副作用
```
...
```javascript
setupRequestGuard(axios, {
  defaults: { duplicate: { strategy: 'ignore', headerFields: ['X-Tenant-Id', 'X-Store-Id'] } },
  rules: [{ method: 'post', duplicate: true }]
});
// 不同租户的相同请求互不干扰
```
...
setupRequestGuard(axios, {
  rules: [{
    ...
    duplicate: {
      strategy: 'ignore', message: '文件上传中，请勿重复提交',
